...
const validators: Record<keyof FormFields, (value: string, country?: string) => string> = {
  email: (value) => {
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    return emailRegex.test(value) ? '' : 'Invalid email format';
  },
  password: (value) => {
    const regex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[a-zA-Zа-яА-ЯёЁ])[a-zA-Zа-яА-ЯёЁ\d]{8,}$/;
...
const validateField = (name: keyof FormFields, value: string, country?: string) => {
    ...
    <div className="flex flex-col items-center justify-center min-h-svh p-4">
      ...
      >
        ...
        {formFields.map((field) => (
          <div key={field.name}>
            {renderInput(field.name as keyof FormFields, field.label, field.type)}
